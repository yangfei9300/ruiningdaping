...
var timer = setInterval(function() {
	...
	$('#date').html('<span>' + year + '/' + (month + 1) + '/' + data + '</span><span>' + ampm +
		'</span><span>周' + day + '</span>')
...
$('.select-ul').on('click', 'li', function() {
	$(this).addClass('active').siblings('li').removeClass('active').parent().hide().siblings('.select-div')
		.html($(this).html());
...
var startColor = ['#0e94eb', '#c440ef', '#efb013', '#2fda07', '#d8ef13', '#2e4af8', '#0eebc4', '#f129b1', '#17defc',
	'#f86363'
];
var borderStartColor = ['#0077c5', '#a819d7', '#c99002', '#24bc00', '#b6cb04', '#112ee2', '#00bd9c', '#ce078f',
	'#00b2cd', '#ec3c3c'
];
...
function chart1() {
	...
	for (var i = 0; i < data.length; i++) {
		str += '<p><span><i class="legend" style="background:' + startColor[i] + '"></i></span>' + data[i].name +
			'<span class="pie-number" style="color:' + startColor[i] + '">' + data[i].value + '</span>' + Number(data[i]
				.percent).toFixed(2) + '%</p>';
	}
...
$("#barTypes").on('click', 'li', function() {
	...
	// 假设你想请求的接口地址是 'https://api.example.com/data'
...
function chart3(type, chartType,data_1) {
	...
	myChart.on('click', function(params) {
		// params 包含了点击事件的相关信息，比如被点击的数据项的信息
		// 你可以通过 params.data 获取到该数据项的所有数据
